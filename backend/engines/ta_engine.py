...
class TAEngine:
    ...
    def _synthetic_ohlcv(self, pair):
    ...
    def _compute_all(self, ohlcv, pair, timeframe):
        c = ohlcv["closes"]
        h = ohlcv["highs"]
        l = ohlcv["lows"]
        o = ohlcv["opens"]
        ...
        indicators = [
            ...
            {"name": "EMA 20/50", "value": "GOLDEN CROSS" if ema20 > ema50 else "DEATH CROSS", "signal": "bull" if ema20 > ema50 else "bear", "progress": 75 if ema20 > ema50 else 25},
            {"name": "Bollinger Bands", "value": "LOWER" if price <= bb_lower else "UPPER" if price >= bb_upper else "MID", "signal": "bull" if price <= bb_lower else "bear" if price >= bb_upper else "neut", "progress": 50},
            {"name": "SMA 20", "value": "ABOVE" if price > sma20 else "BELOW", "signal": "bull" if price > sma20 else "bear", "progress": 65 if price > sma20 else 35},
            {"name": "Trend", "value": "UPTREND" if c[-1] > c[-10] else "DOWNTREND", "signal": "bull" if c[-1] > c[-10] else "bear", "progress": 70 if c[-1] > c[-10] else 30},
            {"name": "EMA 200", "value": "ABOVE" if price > ema200 else "BELOW", "signal": "bull" if price > ema200 else "bear", "progress": 72 if price > ema200 else 28},
        ]
        ...
        if c[-2] < o[-2] and c[-1] > o[-1] and c[-1] > o[-2]:
            patterns.append({"name": "Bullish Engulfing", "type": "bullish", "confidence": 84})
        support = [round(min(l[i:i+5]), 5) for i in range(0, 20, 5)]
        resistance = [round(max(h[i:i+5]), 5) for i in range(0, 20, 5)]
        smc = [
            {"concept": "Market Structure", "description": "BOS Confirmed", "bias": "bullish" if overall == "BULLISH" else "bearish"},
            {"concept": "Order Block", "description": f"OB @ {round(min(l[-10:]), 5)}", "bias": "bullish"},
            {"concept": "Liquidity", "description": f"Liquidity @ {round(max(h[-20:]), 5)}", "bias": "neutral"},
        ]
        return {
